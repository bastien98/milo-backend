...
PRODUCTS = {
    ...
    "Chips": [
        ...
        (
            "Lays Oven Baked Naturel 150g",
            "chips oven baked naturel",
            "lay's",
            2.29,
            2,
            "Chips",
    ...
    "Frozen Pizza": [
        ...
        (
            "Buitoni Bella Napoli 4 Fromaggi",
            "pizza",
            "buitoni",
            4.49,
            2,
    ...
    "Baby Diapers": [
        ...
        (
            "Pampers Premium Protection 4 82st",
            "luiers",
            "pampers",
            27.99,
            None,
    ...
    "Baby Wipes": [
        (
            "Pampers Sensitive Billendoekjes 52st",
            "billendoekjes",
            "pampers",
            3.49,
            None,
    ...
    "Baby Food": [
        ...
        (
            "Bledina Petit Pot Legumes 2x200g",
            "babyvoeding",
            "bledina",
            3.29,
            4,
        ...
        (
            "Ella's Kitchen Fruit Pouch 120g",
            "fruitknijper",
            "ella's kitchen",
            1.99,
            4,
    ...
    "Kids Yogurt": [
        (
            "Danone Gervais Petit Suisse 6x50g",
            "petit suisse",
            "danone",
            2.29,
            3,
    ...
    "Protein Bars": [
        (
            "Barebells Protein Bar Caramel Cashew 55g",
            "protein bar",
            "barebells",
            2.99,
            4,
        ...
        (
            "Barebells Protein Bar Cookies & Cream 55g",
            "protein bar",
            "barebells",
            2.99,
            4,
        ...
        (
            "PhD Smart Bar Chocolate Brownie 64g",
            "protein bar",
            "phd",
            2.79,
            4,
    ...
    "Protein Drinks": [
        (
            "Barebells Milkshake Chocolate 330ml",
            "protein shake",
            "barebells",
            2.99,
            4,
        ...
        (
            "Barebells Milkshake Vanilla 330ml",
            "protein shake",
            "barebells",
            2.99,
            4,
        ...
        (
            "Multipower Protein Shake 330ml",
            "protein shake",
            "multipower",
            2.49,
            4,
        ...
        (
            "Optimum Nutrition Protein Shake 330ml",
            "protein shake",
            "optimum nutrition",
            3.49,
            4,
...
def generate_shopping_data(
    ...
    for receipt_date in receipt_dates:
        ...
        receipt_total = 0.0
        shop_time = datetime.combine(receipt_date, datetime.min.time()) + timedelta(
...
async def rebuild_enriched_profile(user_id: str):
    ...
    try:
        ...
        sorted_items = sorted(
            item_data.items(), key=lambda x: x[1]["total_spend"], reverse=True
        )

        promo_interest_items = []
        for name, data in sorted_items[:25]:
            granular_cat = (
                list(data["granular_categories"])[0]
                if data["granular_categories"]
                else None
            )
            sum(data["health_scores"]) / len(data["health_scores"]) if data[
                "health_scores"
            ] else None
            ...
            promo_interest_items.append(
                {
                    ...
                    "granular_category": granular_cat,
                    "tags": [],
                    "last_purchased": max(data["dates"]).isoformat()
                    if data["dates"]
                    else None,
                    "days_since_last_purchase": (date.today() - max(data["dates"])).days
                    if data["dates"]
                    else None,
        ...
        receipt_count = await conn.fetchval(
            "SELECT COUNT(*) FROM receipts WHERE user_id = $1", user_id
        )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create test user with synthetic Belgian shopping data"
    )
    parser.add_argument(
        "--persona",
        "-p",
        default="mixed",
        help="Shopper persona (health_conscious, budget_shopper, indulgent, family_shopper, mixed)",
    )
    parser.add_argument(
        "--months",
        "-m",
        type=int,
        default=3,
        help="Number of months of shopping history",
    )
    ...
    parser.add_argument(
        "--firebase-uid",
        "-f",
        help="Firebase UID (optional, will generate if not provided)",
    )
    parser.add_argument(
        "--list-personas", "-l", action="store_true", help="List available personas"
    )
    parser.add_argument(
        "--rebuild-profile",
        "-r",
        action="store_true",
        help="Rebuild enriched profile after creating user",
    )
    ...
    if args.rebuild_profile:
        ...
        print(
            f"  Or upload a receipt through the app with firebase_uid: {result['firebase_uid']}"
        )
