...
if user_input_id:
    ...
    raise ValueError(
        "User ID cannot be empty. Please run the script again and provide a valid ID."
    )
...
PRODUCTS = {
    "ALCOHOL": [
        ("JUPILER BAK 24X25CL", 14.99, 0),
        ("STELLA ARTOIS 24X25CL", 15.49, 0),
        ("CARA PILS 24X33CL", 9.99, 0),
        ("DUVEL 4X33CL", 6.99, 0),
        ("LEFFE BLOND 6X33CL", 7.49, 0),
        ("VODKA SMIRNOFF 70CL", 18.99, 0),
        ("WHISKY JACK DANIELS 70CL", 24.99, 0),
        ("GORDON'S GIN 70CL", 19.99, 0),
        ("RODE WIJN BORDEAUX", 6.99, 1),
        ("WITTE WIJN CHARDONNAY", 5.99, 1),
    ],
    "TOBACCO": [
        ("MARLBORO RED", 8.50, 0),
        ("CAMEL BLUE", 8.20, 0),
        ("L&M RED", 7.90, 0),
        ("ROLLING TOBACCO DRUM 50G", 12.50, 0),
    ],
    "SNACKS_SWEETS": [
        ("CHIPS LAYS PAPRIKA", 1.99, 1),
        ("PRINGLES ORIGINAL", 2.49, 1),
        ("CHOCOLATE COTE D'OR", 2.99, 1),
        ("HARIBO GOLDBEARS", 1.89, 1),
        ("BEN & JERRYS COOKIE DOUGH", 5.99, 1),
    ],
    "READY_MEALS": [
        ("LASAGNE BOLOGNESE 400G", 3.99, 2),
        ("PIZZA MARGHERITA", 2.99, 2),
        ("SOEP TOMATEN", 2.49, 3),
        ("FRIKANDEL 4ST", 2.19, 1),
    ],
    "DRINKS_SOFT_SODA": [
        ("COCA COLA 1.5L", 2.19, 1),
        ("FANTA ORANGE 1.5L", 2.19, 1),
        ("RED BULL 25CL", 1.49, 0),
    ],
    "DRINKS_WATER": [
        ("WATER SPA BLAUW 6X1.5L", 4.99, 5),
        ("WATER SPA ROOD 6X1.5L", 4.99, 5),
    ],
    "MEAT_FISH": [
        ("GEHAKT 500G", 4.99, 3),
        ("KIPFILET 1KG", 9.99, 4),
        ("BURGER 2ST", 3.49, 2),
        ("ZALMFILET 200G", 5.99, 5),
    ],
    "DAIRY_EGGS": [
        ("MELK HALFVOL 1L", 0.99, 4),
        ("EIEREN 12ST", 2.49, 4),
        ("KAAS GOUDA", 4.99, 3),
        ("YOGHURT DANONE", 2.99, 3),
    ],
    "FRESH_PRODUCE": [
        ("BANANEN 1KG", 1.99, 5),
        ("APPELS JONAGOLD", 2.49, 5),
        ("TOMATEN", 1.99, 5),
        ("KOMKOMMER", 0.89, 5),
        ("WORTELEN 1KG", 1.29, 5),
        ("SLA MIX", 1.99, 5),
    ...
    "HOUSEHOLD": [
        ("TOILETPAPIER 12ROL", 5.99, 0),
        ("AFWASMIDDEL DREFT", 2.99, 0),
        ("VUILNISZAKKEN", 3.49, 0),
    ],
    "BAKERY": [("BAGUETTE", 1.19, 3), ("CROISSANTS 4ST", 3.49, 2)],
    "PANTRY": [
        ("PASTA SPAGHETTI 500G", 1.29, 3),
        ("OLIJFOLIE 50CL", 6.99, 4),
        ("RIJST 1KG", 2.49, 4),
    ],
...
def generate_user_data():
    ...
    w_start = np.array(
        [0.25, 0.20, 0.15, 0.02, 0.05, 0.05, 0.05, 0.05, 0.08, 0.05, 0.03, 0.02]
    )
    w_end = np.array(
        [0.05, 0.05, 0.05, 0.10, 0.15, 0.15, 0.25, 0.05, 0.05, 0.05, 0.03, 0.02]
    )

    print(f"Generating data from {START_DATE.date()} to {END_DATE.date()}...")
    print(
        f"Targeting ~{AVG_RECEIPTS_PER_MONTH} receipts/month (Daily prob: {daily_prob:.2f})"
    )
    ...
    while current_date <= END_DATE:
        ...
        if random.random() < daily_prob:
            ...
            shop_time = current_date + timedelta(
                ...
                transactions_list.append(
                    {
                        ...
                        "created_at": shop_time.strftime("%Y-%m-%d %H:%M:%S.%f")
                        + " +00:00",
            ...
            receipts_list.append(
                {
                    ...
                    "created_at": upload_time.strftime("%Y-%m-%d %H:%M:%S.%f")
                    + " +00:00",
                    "processed_at": process_time.strftime("%Y-%m-%d %H:%M:%S.%f")
                    + " +00:00",
                }
            )
