...
from ai.promo_pipelines.models import PromoItem
from ai.promo_pipelines.prompt_builder import build_system_prompt
from ai.promo_pipelines.stores import load_store_config
...
def extract_promos_from_pdf(pdf_path: Path, config: Dict[str, Any]) -> dict:
    """Split PDF into batches and extract promo items sequentially."""
    batches = split_pdf_into_batches(pdf_path)
...
def run_pipeline(
    ...
    Args:
        ...
        pdf_path: Path to the promo folder PDF
    ...
    logger.info(f"Store: {store_id} ({display_name})")
    ...
    if clear_index and not dry_run:
        clear_all_retailer_promos(store_id)

    # Step 1: Extract from PDF via Gemini
    raw_data = extract_promos_from_pdf(pdf_path, config)

    # Step 2: Parse and validate
    items = parse_promo_items(raw_data, store_id, promo_folder_url)
