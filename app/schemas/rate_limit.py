...
class RateLimitStatusResponse(BaseModel):
    ...
    messages_remaining: int = Field(
        ..., description="Messages remaining in current period"
    )
    receipts_used: int = Field(
        ..., description="Number of receipt uploads used in current period"
    )
    receipts_limit: int = Field(
        ..., description="Maximum receipt uploads allowed per period"
    )
    receipts_remaining: int = Field(
        ..., description="Receipt uploads remaining in current period"
    )
    period_start_date: datetime = Field(
        ..., description="Start of current rate limit period (UTC)"
    )
    period_end_date: datetime = Field(
        ..., description="End of current rate limit period (UTC)"
    )
...
class RateLimitExceededResponse(BaseModel):
    ...
    period_end_date: datetime = Field(
        ..., description="When the rate limit resets (UTC)"
    )
