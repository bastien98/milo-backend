...
class PromoResult(BaseModel):
    ...
    original_description: str = Field(
        ..., description="Original description from retailer"
    )
    ...
    promo_mechanism: Optional[str] = Field(
        None, description="Type of promotion (e.g., '1+1', '-30%')"
    )
    ...
    relevance_score: float = Field(
        ..., description="How relevant this promo is to the query (0-1)"
    )
...
class SearchQuery(BaseModel):
    ...
    product_keywords: List[str] = Field(
        default_factory=list, description="Product-related keywords"
    )
    brands: List[str] = Field(
        default_factory=list, description="Specific brands mentioned"
    )
    categories: List[str] = Field(
        default_factory=list, description="Product categories"
    )
    granular_categories: List[str] = Field(
        default_factory=list, description="3 granular category guesses for filtering"
    )
    retailers: List[str] = Field(
        default_factory=list, description="Specific retailers/stores"
    )
    is_vague: bool = Field(False, description="Whether the query needs clarification")
    clarification_needed: Optional[str] = Field(
        None, description="What to ask the user for clarification"
    )
...
class PromoChatResponse(BaseModel):
    ...
    promos: List[PromoResult] = Field(
        default_factory=list, description="Matching promotions"
    )
    search_query: Optional[SearchQuery] = Field(
        None, description="The structured query used for search"
    )
    needs_clarification: bool = Field(
        False, description="Whether more user input is needed"
    )
